...
import Steiner.Language.Error (ErrorSource(..), InferenceErrors, notInScope, propagateErrors)
...
infer ast = propagateErrors (ValueError ast) $ infer' ast

-- |
-- Internal version of infer which doesn't add the current location to error stacks
--
infer' :: Expression -> InferT InferErrorM Type
